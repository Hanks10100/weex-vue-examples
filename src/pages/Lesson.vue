...
<script>
  ...
  export default {
    data () {
      const subjectRE = /.+\/?[\?\&]{1}subject=([\d\w]+)$/i
      const match = subjectRE.exec(weex.config.bundleUrl || '')
      let subject = 'weex'
      if (match) {
        subject = match[1]
      }
      let chosenLesson = learnWeex
      switch (subject.toLowerCase()) {
        case 'vue': chosenLesson = learnVue; break
        case 'javascript':
        case 'js': chosenLesson = learnJS; break
        case 'css': chosenLesson = learnCSS; break
        case 'hackernews': chosenLesson = hackernewsApp; break
      }
