<template>
  <div>
    ...
    <div v-for="r in row" style="flex-direction: row; flex: 1;" :key="r">
      <div v-for="c in col" style="flex: 1;" :key="c">
        <text :tid="tid" @click="onclick" @longpress="onlongpress" :class="[state, 'tile']" :around="around">{{c}}</text>
      </div>
    </div>
    <text @click="restart" class="btn">START</text>
  </div>
</template>
...
module.exports = {
  ...
  methods: {
    map: function (x, y, callback) {
      // visit tiles around (x, y)
      for (var i = 0; i < 8; ++i) {
    ...
    position: function (tid) {
      // return (x, y) with tile id
    ...
    tile: function (event) {
      // return tile object with click event
